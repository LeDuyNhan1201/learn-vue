...
export function useGetStatusesQuery() {
  return useQuery<StatusesResponse, ResourceNotFoundErrorResponse>({
    queryKey: ["statuses"],
    queryFn: () => getStatuses(),
...
export function useGetTasksQuery() {
  return useQuery<TasksResponse, ResourceNotFoundErrorResponse>({
    queryKey: ["tasks"],
    queryFn: () => getTasks(),
    throwOnError: (error) => isAxiosError(error),
    retry: 1,
...
export function useGetTaskDetailsQuery(
  param: () => TaskIdPathParam | null,
  options?: Omit<Parameters<typeof useQuery>[0], "queryKey" | "queryFn">,
) {
  return useQuery<TaskItem | null, ResourceNotFoundErrorResponse>({
    queryKey: ["task-details", param],
    queryFn: () => {
      const id = param();
      if (!id) return null;
